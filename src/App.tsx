...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const prepaidActivitiesMyr = activityBudgets
      .filter((line) => line.kind === "paid")
      .reduce((sum, line) => sum + line.myr, 0);
    const prepaidMyr = prepaidActivitiesMyr + accommodationsMyr;
    const onSiteMyr = totalMyr - prepaidMyr;
    const officialLines = activityBudgets.filter((line) => line.kind === "official").length;
    const categories = activityBudgets
      .filter((line) => line.kind !== "paid")
    ...
    return {
      ...
      officialLines,
      categories,
      averagePerDay: onSiteMyr / tripData.days.length,
    };
  }, []);
